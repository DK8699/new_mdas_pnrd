...
@section('content')
    <div class="row">
        <ol class="breadcrumb">
            ...
            <li class="active"><a href="{{ route('pris.members.bankReport') }}"> Back To Bank Records</a></li>
        </ol>
    </div>
                ...
                <tbody>
                    ...
                    @foreach ($apcount as $ap)
                        ...
                        <tr>
                            ...
                            {{-- <td>{{ $ap->id }}</td> --}}
                            <td>
                                <a href="{{ route('pris.members.bankSubDistrictGP', $ap->id) }}"> {{$ap->anchalik_parishad_name }}</a>
                            </td>

                            <td>{{ $apStrength[$i - 1][0]->myapStrength }}</td>
                            <td>{{ $apBankRecordCount[$i - 1][0]->mycountap }}</td>
                            <td>{{ $apStrength[$i - 1][0]->myapStrength * 013 }}</td>
