...
<div class="row">
        <ol class="breadcrumb">
            ...
            <li class="active">Asset Settlement</li>
        </ol>
</div>
        ...
        <div class="col-md-12 col-sm-12 col-xs-12 mt10">
            <div class="panel panel-primary">
                    <div class="panel-heading" style="text-align: center">
                        Settlement List of Zila Parishad : {{$district_name}} and its Anchalik Panchayats ({{$fy_years}})
                    </div>
                            ...
                            <table class="table table-bordered table-striped" id="dataTable1">
                                <thead>
                                 ...
                                 <tbody>
                                   ...
                                   <tr>
                                       <td>1</td>
                                       <td><a href="{{route('admin.Osr.osrAssetBranchList',[$id,$fy_id])}}">{{$district_name}}</a></td>
                                       <td class="text-center">
                                        @if(isset($dataCount['zpAssetCount'][$id]))
                                                {{$dataCount['zpAssetCount'][$id]}}
                                        @else
                                                {{0}}
                                        @endif
                                       </td>
                                       ...
                                       @if(isset($dataCount['zpAssetCount'][$id]))
                                            @if(isset($dataCount['zpSettledCount'][$id]))
                                                {{round((($dataCount['zpSettledCount'][$id]/$dataCount['zpAssetCount'][$id])*100), 2)}}
                                            @else
                                                    {{'0'}}
                                            @endif
                                        @else
                                ...
                                @foreach($ap_list as $ap)
                                   <tr>
                                       <td>{{$i++}}</td>
                                       <td>
                                           <a href="{{route('admin.Osr.subAPWiseAssetSettlement',[$id,$fy_id,$ap->id])}}">
                                               {{$ap->anchalik_parishad_name}}</a>
                                       </td>
                                       <td class="text-center">
                                       @if(isset($dataCount['totalAssetCount'][$ap->id]))
                                                {{$dataCount['totalAssetCount'][$ap->id]}}
                                           @php $totalAsset=$totalAsset+$dataCount['totalAssetCount'][$ap->id]; @endphp
                                       @else
                                                {{0}}
                                       @endif
                                       </td>
                                       ...
                                       @if(isset($dataCount['totalAssetCount'][$ap->id]))
                                            @if(isset($dataCount['apYrWiseSettledAssetCount'][$ap->id]))
                                                {{round(($dataCount['apYrWiseSettledAssetCount'][$ap->id]/$dataCount['totalAssetCount'][$ap->id])*100, 2)}}
                                            @else
                                                {{'0'}}
                                            @endif
                                        @else
        ...
        $(document).ready(function () {
            $('#dataTable1').DataTable({
                ...
                buttons: [
                    {
                        extend:    'excelHtml5',
                        title: 	   'Settlement List of Zila Parishad : {{$district_name}} and its Anchalik Panchayats ({{$fy_years}})',
