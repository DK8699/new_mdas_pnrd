    ...
    <div class="row mt40">
        ...
        <div class="panel panel-primary">
            <div class="panel-heading" style="text-align: center">
                Settlement Percentage for Anchalik Panchayats : {{$data['zpData']->zila_parishad_name}} ({{$data['fyData']->fy_name}})
            </div>
                    ...
                    <table class="table table-bordered table-striped" id="dataTable1">
                        ...
                        @foreach($data['apList'] as $ap)
                            <tr>
                                <td>{{$i}}</td>
                                <td class="text-left">
                                    <a href="{{route('osr.non_tax.asset.common.branch_list_settlement_defaulter', [encrypt($data['fy_id']), encrypt('SETTLEMENT'), encrypt('AP'), encrypt($data['zpData']->id), encrypt($ap->id), encrypt('NULL')])}}">
                                        {{$ap->anchalik_parishad_name}}
                                    </a>
                                </td>
                                <td>
                                    @if(isset($data['totalScope'][$ap->id]))
                                        {{$data['totalScope'][$ap->id]}}</td>
                                    @else
                                        {{'0'}}
                                    @endif
                                <td>
                                    @if(isset($data['shortlist'][$ap->id]))
                                        {{$data['shortlist'][$ap->id]}}
                                    @else
                                        {{'0'}}
                                    @endif
                                </td>
                                ...
                                @if(isset($data['shortlist'][$ap->id]))
                                            @if(isset($data['settled'][$ap->id]))
                                                {{round((($data['settled'][$ap->id]/$data['shortlist'][$ap->id])*100), 2)}}
                                            @else
                                                    {{'0'}}
                                            @endif
                                        @else
        ...
        $(document).ready(function () {
            $('#dataTable1').DataTable({
                ...
                buttons: [
                                {
                                    extend:    'excelHtml5',
                                    title:     'Settlement Percentage for Anchalik Panchayats : {{$data['zpData']->zila_parishad_name}} ({{$data['fyData']->fy_name}})',
