        ...
        <div class="row mt40">
            ...
            <div class="panel panel-primary">
                <div class="panel-heading" style="text-align: center">
                    Settlement Percentage for Anchalik Panchayat: {{$data['apData']->anchalik_parishad_name}} under Zila Parishad : {{$data['zpData']->zila_parishad_name}} ({{$data['fyData']->fy_name}})
                </div>
                        ...
                        <table class="table table-bordered table-striped" id="dataTable1">
                            ...
                            @foreach($data['branches'] as $branch)
                                <tr >
                                    <td>{{$i}}</td>
                                    <td class="text-left">
                                        <a href="{{url('osr/non_tax/asset/zp/ap_asset_settlement_percent_branch')}}/{{encrypt($data['fy_id'])}}/{{encrypt($data['apData']->id)}}/{{encrypt($branch->id)}}">
                                            {{$branch->branch_name}}
                                        </a>
                                    </td>
                                    <td>52</td>
