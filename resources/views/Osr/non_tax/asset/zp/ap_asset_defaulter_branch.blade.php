        ...
        <div class="row mt40">
            ...
            <div class="panel panel-primary">
                <div class="panel-heading" style="text-align: center">
                   Defaulter of {{$data['branchData']->branch_name}} of Anchalik Panchayat : {{$data['apData']->anchalik_parishad_name}} under Zila Parishad : {{$data['zpData']->zila_parishad_name}} ({{$data['fyData']->fy_name}})
                </div>
                        ...
                        <table class="table table-bordered table-striped" id="dataTable1">
                            ...
                            @foreach($data['assetList'] as $li)
                                <tr>
                                    <td>{{$i}}</td>
                                    <td class="text-left">
                                        <a href="{{route('Osr.non_tax.dwAssetInformation')}}">{{$li->asset_code}}</a>
                                    </td>
                                    <td class="text-left">
                                        {{$li->asset_name}}
                                    </td>
