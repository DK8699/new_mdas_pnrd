        ...
        <div class="row mt40">
            ...
            <div class="panel panel-primary">
                <div class="panel-heading" style="text-align: center">
                   Revenue Collection for Gram Panchayat : {{$data['gpData']->gram_panchayat_name}} under Anchalik Panchayat: {{$data['apData']->anchalik_parishad_name}} under District : {{$data['zpData']->zila_parishad_name}} ({{$data['fyData']->fy_name}})
                </div>
                        ...
                        <table class="table table-bordered table-striped" id="dataTable1">
                            ...
                            @foreach($data['branches'] as $branch)
                                <tr >
                                    <td>{{$i}}</td>
                                    <td class="text-left">
                                        <a href="{{url('osr/non_tax/asset/zp/gp_asset_collection_branch')}}/{{encrypt($data['fy_id'])}}/{{encrypt($data['ap_id'])}}/{{encrypt($data['gp_id'])}}/{{encrypt($branch->id)}}">
                                            {{$branch->branch_name}}
                                        </a>
                                    </td>
                                    <td>5000</td>
