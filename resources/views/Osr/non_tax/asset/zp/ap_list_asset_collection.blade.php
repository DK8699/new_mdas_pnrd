    ...
    <div class="row mt40">
        ...
        <div class="panel panel-primary">
            <div class="panel-heading" style="text-align: center">
                Assets(Haat, Ghat, Fishery, Animal Pound) Revenue Collection of Anchalik Panchayats of Zila Parishad : {{$data['zpData']->zila_parishad_name}} ({{$data['fyData']->fy_name}})
            </div>
                    ...
                    <table class="table table-bordered table-striped" id="dataTable1">
                        ...
                        @foreach($data['apList'] as $ap)
                            <tr>
                                <td>{{$i}}</td>
                                <td>
                                    <a href="{{route('osr.non_tax.asset.common.branch_list_revenue_share', [encrypt($data['fy_id']), encrypt("REVENUE"), encrypt("AP"), encrypt($data['zpData']->id), encrypt($ap->id), encrypt("NULL")])}}">
                                        {{$ap->anchalik_parishad_name}}
                                    </a>
                                </td>
                                <td class="text-right">
                                    <span class="money_txt">
                                        {{$data['resArray'][$ap->id]['gap_c']}}
                                    </span>
                                </td>
                                <td class="text-right">
                                    <span class="money_txt">
                                        {{$data['resArray'][$ap->id]['bid_c']}}
                                    </span>
                                </td>
                                <td class="text-right">
                                    <span class="money_txt">
                                        {{$data['resArray'][$ap->id]['tot_c']}}
                                    </span>
                                </td>
                            </tr>
                            @php $i++; @endphp
                            ...
                            @php
                                $tot_c=$tot_c+$data['resArray'][$ap->id]['tot_c'];
                                $gap_c=$gap_c+$data['resArray'][$ap->id]['gap_c'];
                                $bid_c=$bid_c+$data['resArray'][$ap->id]['bid_c'];
                            @endphp
                        @endforeach
        ...
        $(document).ready(function () {
            $('#dataTable1').DataTable({
                ...
                buttons: [
                    {
                        extend:    'excelHtml5',
                        title: 	   'Assets(Haat, Ghat, Fishery, Animal Pound) Revenue Collection of Anchalik Panchayats of Zila Parishad : {{$data['zpData']->zila_parishad_name}} ({{$data['fyData']->fy_name}})',
