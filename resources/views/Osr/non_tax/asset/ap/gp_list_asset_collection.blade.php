        ...
        <div class="row mt40">
            ...
            <div class="panel panel-primary">
                <div class="panel-heading" style="text-align: center">
                    Assets(Haat, Ghat, Fishery, Animal Pound) Revenue Collection of Gram Panchayats of Anchalik Panchayat : {{$data['apData']->anchalik_parishad_name}} , Zila Parishad : {{$data['zpData']->zila_parishad_name}} ({{$data['fyData']->fy_name}})
                </div>
                        ...
                        <table class="table table-bordered table-striped" id="dataTable1">
                            ...
                            @foreach($data['gpList'] as $gp)
                                <tr>
                                    ...
                                    <td class="text-left">
                                        <a href="{{route('osr.non_tax.asset.common.branch_list_revenue_share', [encrypt($data['fy_id']), encrypt("REVENUE"), encrypt("GP"), encrypt($data['zpData']->id), encrypt($data['apData']->id), encrypt($gp->id)])}}">
                                            {{$gp->gram_panchayat_name}}
                                        </a>
                                    </td>
                                    <td class="text-right">
                                    <span class="money_txt">
                                        {{$data['resArray'][$gp->id]['gap_c']}}
                                    </span>
                                    </td>
                                    <td class="text-right">
                                    <span class="money_txt">
                                        {{$data['resArray'][$gp->id]['bid_c']}}
                                    </span>
                                    </td>
                                    <td class="text-right">
                                    <span class="money_txt">
                                ...
                                @php
                                    ...
                                    $gap_c=$gap_c+$data['resArray'][$gp->id]['gap_c'];
                                    $bid_c=$bid_c+$data['resArray'][$gp->id]['bid_c'];
                                @endphp
                            @endforeach
        ...
        $(document).ready(function () {
            $('#dataTable1').DataTable({
                ...
                buttons: [
                    {
                        extend:    'excelHtml5',
                        title: 	   'Assets(Haat, Ghat, Fishery, Animal Pound) Revenue Collection of Gram Panchayats of Anchalik Panchayat : {{$data['apData']->anchalik_parishad_name}} , Zila Parishad : {{$data['zpData']->zila_parishad_name}} ({{$data['fyData']->fy_name}})',
