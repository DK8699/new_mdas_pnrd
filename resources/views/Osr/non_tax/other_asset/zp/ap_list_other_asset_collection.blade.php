    ...
    <div class="row mt40">
        ...
        <div class="panel panel-primary">
            <div class="panel-heading" style="text-align: center">
                Other Assets Revenue Collection of Anchalik Panchayats of Zila Parishad : {{$data['zpData']->zila_parishad_name}} ({{$data['fyData']->fy_name}})
            </div>
                    ...
                    <table class="table table-bordered table-striped" id="dataTable1">
                        ...
                        @foreach($data['apList'] as $ap)
                            <tr>
                                <td>{{$i}}</td>
                                <td>
                                    <a href="{{route('osr.non_tax.other_asset.common.cat_list_revenue_share', [encrypt($data['fy_id']), encrypt("REVENUE"), encrypt("AP"), encrypt($data['zpData']->id), encrypt($ap->id), encrypt("NULL")])}}">
        ...
        $(document).ready(function () {
            $('#dataTable1').DataTable({
                ...
                buttons: [
                    {
                        extend:    'excelHtml5',
                        title: 	   'Other Assets Revenue Collection of Anchalik Panchayats of Zila Parishad : {{$data['zpData']->zila_parishad_name}} ({{$data['fyData']->fy_name}})',
