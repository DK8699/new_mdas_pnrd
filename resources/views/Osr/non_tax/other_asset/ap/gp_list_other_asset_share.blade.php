    ...
    <div class="row mt40">
        ...
        <div class="panel panel-primary">
            <div class="panel-heading" style="text-align: center">
                Other Asset Share distribution of Gram Panchayats of Anchalik Panchayat: {{$data['apData']->anchalik_parishad_name}}, Zila Parishad : {{$data['zpData']->zila_parishad_name}} ({{$data['fyData']->fy_name}})
            </div>
                    ...
                    <table class="table table-bordered table-striped" id="dataTable1">
                        ...
                        @foreach($data['gpList'] as $gp)
                            <tr>
                                <td>{{$i}}</td>
                                <td class="text-left">
                                    <a href="{{route('osr.non_tax.other_asset.common.cat_list_revenue_share', [encrypt($data['fy_id']), encrypt("SHARE"), encrypt("GP"), encrypt($data['zpData']->id), encrypt($data['apData']->id), encrypt($gp->id)])}}">
                                        {{$gp->gram_panchayat_name}}
                                    </a>
                                </td>
                                <td class="text-right">
                                    <span class="money_txt">
                                        {{$data['resArray'][$gp->id]['tot_r_c']}}
                                    </span>
                                </td>
                                <td class="text-right">
                                    <span class="money_txt">
                                        {{$data['resArray'][$gp->id]['zp_share']}}
                                    </span>
                                </td>
                                <td class="text-right">
                                    <span class="money_txt">
                                        {{$data['resArray'][$gp->id]['ap_share']}}
                                    </span>
                                </td>
                                <td class="text-right">
                                    <span class="money_txt">
                                        {{$data['resArray'][$gp->id]['gp_share']}}
                                    </span>
                                </td>
                            </tr>
                            @php $i++; @endphp
                            @php
                                $tot_r_c=$tot_r_c+$data['resArray'][$gp->id]['tot_r_c'];
                                $zp_share=$zp_share+$data['resArray'][$gp->id]['zp_share'];
                                $ap_share=$ap_share+$data['resArray'][$gp->id]['ap_share'];
                                $gp_share=$gp_share+$data['resArray'][$gp->id]['gp_share'];
                            @endphp
                        @endforeach
        ...
        $(document).ready(function () {
            $('#dataTable1').DataTable({
                ...
                buttons: [
                    {
                        extend:    'excelHtml5',
                        title: 	   'Other Asset Share distribution of Gram Panchayats of Anchalik Panchayat: {{$data['apData']->anchalik_parishad_name}}, Zila Parishad : {{$data['zpData']->zila_parishad_name}} ({{$data['fyData']->fy_name}})',
