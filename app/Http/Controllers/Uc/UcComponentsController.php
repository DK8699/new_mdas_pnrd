...
class UcComponentsController extends Controller
{
    ...
    public function add_edit_component_details(Request $request) {
        ...
        return view('Uc.ComponentsDetails', compact('project', 'zilas','extension_centre'));
     }
    ...
    public function load_entities(Request $request) {
        if($request->ajax())
        {
            ...
            {
                $whereArray = [
                    ['uc_components_entities.project_year_id','=',$py_id],
                    ['div.division_type','=',2],
                    ['div.zilla_extension_id','=',$users->zp_id],
                ];
            }
            else if($users->mdas_master_role_id==6)
            {
                $whereArray = [
                    ['uc_components_entities.project_year_id','=',$py_id],
                    ['div.division_type','=',1],
                    ['div.zilla_extension_id','=',$users->ex_id],
                ];
            }
     ...
     public function get_entity_components(Request $request)
    {
          ...
          if( Empty($components) )
          {
               return view('Uc.addEditComponentsDetails', compact('entity_id', 'components_headers', 'components', 'components_details'));
          }
          else
          {
               return view('Uc.addEditComponentsDetails', compact('entity_id', 'components_headers', 'components', 'components_details'));
    ...
    public function view_component_details(Request $request) {
	   ...
	   $project = UcProjectEntry::join('uc_project_divisions as p_div','p_div.project_id','=','uc_project_entries.id')
        ...
        return view('Uc.listComponentsDetails', compact('project', 'zilas','extension_centre'));
     }
     ...
     public function add_component(){
       ...
       /* $uc_components = UcComponent::getComponents();
        ...
        echo json_encode($uc_components);*/
         ...
         return view('Uc.add_new_component',compact('header','uc_components'));
    }
     ...
     public function view_entity_components($id)
    {
          ...
          if( Empty($components) )
          {
               return view('Uc.viewComponentsDetails', compact('entity_id', 'components_headers', 'components', 'components_details'));
          }
          else
          {
               return view('Uc.viewComponentsDetails', compact('entity_id', 'components_headers', 'components', 'components_details'));
     ...
     public function show_entity_components(Request $request)
    {
          ...
          if( Empty($components) )
          {
               return view('Uc.showComponentsDetails', compact('entity_id', 'components_headers', 'components', 'components_details'));
          }
          else
          {
               return view('Uc.showComponentsDetails', compact('entity_id', 'components_headers', 'components', 'components_details'));
