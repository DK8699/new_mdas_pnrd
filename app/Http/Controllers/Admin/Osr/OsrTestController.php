...
class OsrTestController extends Controller
{
    ...
    public function osr_testing(Request $request){
...
//revenue data*****************************************************
  ...
  //revinue data end*****************************************************
	...
	//echo json_encode($max_osr_fy_year);
        ...
        return view('admin.Osr.osr_testing', compact('dataCount','zilas','max_osr_fy_year','master_fy_years','encode_fy_id','fy_years','card_data','yrWiseAssetCount','dataPaiChart', 'yrWiseOtherAssetCount'));
    }

//Test Ningthem
public function TestSubDistrictWiseRevenue(Request $request){
    
    $master_fy_years = DB::table('osr_master_fy_years')
    ->select('id','fy_name')->get();

    $temp = end($master_fy_years);
    $lastfy = $temp;

    // testing for the year id parametre
    if(isset($request->id) && $request->fid != '' && $request->fid != NULL){
        $yid = $request->id;
    } 
    else{
      $yid = end($lastfy)->id;
    ...
    $datas = DB::table('osr_non_tax_asset_entries')
    ->join('zila_parishads', 'osr_non_tax_asset_entries.zila_id', '=', 'zila_parishads.id')
    ->join('osr_non_tax_asset_final_records', 'osr_non_tax_asset_entries.asset_code', '=', 'osr_non_tax_asset_final_records.asset_code')
    ->where([
        ['osr_non_tax_asset_final_records.fy_id', '=', $yid],
        ['osr_non_tax_asset_final_records.instalment_completed_status', '=', '0'],
        ['zila_parishads.is_active', '=', '1'],
    ])
    ->groupBy('zila_parishads.id')
    ->orderBy('zila_parishads.zila_parishad_name')
    ->select('zila_parishads.zila_parishad_name', DB::raw('COUNT(osr_non_tax_asset_entries.asset_code) as total'))
    ->get();

   

    // return view('admin.Osr.osr_testing',compact('district_name','fy_years','fy_id','datas'));
    return view('admin.Osr.osr_testing',compact('datas','master_fy_years','lastfy'));
}

public function nonCompletedAssetZps(Request $request){
    $jdata[0] = [
    ...
    if($request->ajax()){
        $datas = DB::table('osr_non_tax_asset_entries')
                ...
                ->join('osr_non_tax_asset_final_records', 'osr_non_tax_asset_entries.asset_code', '=', 'osr_non_tax_asset_final_records.asset_code')
                ->where([
                    ['osr_non_tax_asset_final_records.fy_id', '=', base64_decode($request->fy)],
                    ['osr_non_tax_asset_final_records.instalment_completed_status', '=', '0'],
                    ['zila_parishads.is_active', '=', '1'],
                ])
                ->groupBy('zila_parishads.id')
        ...
        foreach($datas AS $value){
            $jdata[$i] = [
                'zp_name'=>$value->zila_parishad_name,
                'asset_count'=>$value->total
            ];
            $i++;
        }
    }
    ...
    // public function TestSubDistrictWiseRevenue($id, $fy_id){
    ...
    //     return view('admin.Osr.Revenue.TestSubDistrictWiseRevenue',compact('district_name','fy_years','ap_list','id','fy_id','dataCount'));
    // }
	...
	public function notShortlistedReport($f_id,$id,Request $request){
		...
		$notShortlistAsset = OsrNonTaxAssetShortlist::join('osr_non_tax_asset_entries as a_entries','a_entries.asset_code','=','osr_non_tax_asset_shortlists.asset_code')
			->where([
				['osr_non_tax_asset_shortlists.osr_master_fy_year_id',$fy_id],
				['osr_non_tax_asset_shortlists.level','NA'],
				['osr_non_tax_asset_shortlists.zp_id',$zp_id]
			])->select('a_entries.asset_name','a_entries.asset_code','osr_non_tax_asset_shortlists.reason')->get();
